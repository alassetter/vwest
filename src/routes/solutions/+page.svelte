<script>
	import HeroAlt from '$lib/components/content/HeroAlt.svelte';
</script>

<section>
	<HeroAlt
		backgroundColor="bg-gray-900"
		gradient="bg-gradient-bluemesh"
		subhead="Solutions"
		subheadColor="text-gray-300"
		title="How we can help"
		titleColor="text-gray-100"
		bodyCopy="Donec id elit non mi porta gravida at eget metus. Etiam porta sem malesuada magna mollis euismod. Donec id elit non mi porta gravida at eget metus."
		bodyColor="text-gray-300"
		imagePath="/images/placeholder-lg.jpg"
		imageAlt="demo"
		imageWidth=""
		imageHeight=""
		imageClasses="absolute w-full mx-auto left-0 top-0 rounded-xl object-cover h-112 lg:h-196 xl:h-200 aspect-video"
	/>
</section>

<section class="bg-gray-900 text-white">
	<div class="pt-40 md:pt-40 lg:pt-16 xl:pt-100 2xl:pt-100 pb-16">
		<!-- Global Padding -->
		<div class="px-12">
			<div class="w-full max-w-full mx-auto">
				<div class="flex flex-wrap">
					<!-- Content -->
					<div class="w-full md:8/12 lg:w-9/12 xl:w-7/12 2xl:6/12">
						<p class="mb-8 text-base font-medium uppercase">
							Subheadline
						</p>
						<h1 class="mb-8 text-5xl max-w-2xl">
							Aenean lacinia bibendum nulla sed consectetur.
						</h1>
						<p class="text-lg">
							Vestibulum id ligula porta felis euismod semper.
							Maecenas faucibus mollis interdum. Aenean lacinia
							bibendum nulla sed consectetur. Duis mollis, est non
							commodo luctus, nisi erat porttitor ligula, eget lacinia
							odio sem nec elit. Cras justo odio, dapibus ac facilisis
							in, egestas eget quam.
						</p>
					</div>
					<!-- Content -->
				</div>
			</div>
		</div>
	</div>
</section>

<!-- Tech Stack -->
<section class="py-12 md:py-24 bg-coolGray-50">
	<div class="w-full mx-auto px-12">
		<div class="max-w-xl lg:max-w-none mx-auto">
			<div class="flex flex-wrap -mx-4 items-center">
				<div class="w-full lg:w-5/12 px-4 mb-20 lg:mb-0">
					<div class="max-w-lg">
						<span
							class="inline-block mb-4 py-2 px-3 text-sm text-black group-hover:text-white leading-none border-1.5 border-black group-hover:bg-black rounded-full transition duration-200"
							>Technology Stack</span
						>
						<h2
							class="font-heading text-6xl sm:text-7xl tracking-tighter mb-10"
						>
							Curious How We Stack Up?
						</h2>
						<p>
							As a software development company with years of
							experience, we only utilize the best in class
							technologies, tools, and methodologies to provide your
							company with highly scalable and secure solutions.
						</p>
					</div>
				</div>
				<div class="w-full lg:w-7/12 px-4">
					<div class="max-w-xl lg:ml-auto">
						<div class="max-w-lg">
							<span class="mr-4">I will become a sponsor</span>
						</div>
						<div class="flex flex-wrap -mx-4 -mb-4">
							<div class="w-full xs:w-1/2 sm:w-1/3 px-4 mb-4">
								<div
									class="flex h-28 p-2 items-center justify-center bg-white rounded-3xl"
								>
									<img
										class="block mx-auto"
										src="/V-logo.svg"
										alt=""
									/>
								</div>
							</div>
							<div class="w-full xs:w-1/2 sm:w-1/3 px-4 mb-4">
								<div
									class="flex h-28 p-2 items-center justify-center bg-white rounded-3xl"
								>
									<img
										class="block mx-auto"
										src="/V-logo.svg"
										alt=""
									/>
								</div>
							</div>
							<div class="w-full xs:w-1/2 sm:w-1/3 px-4 mb-4">
								<div
									class="flex h-28 p-2 items-center justify-center bg-white rounded-3xl"
								>
									<img
										class="block mx-auto"
										src="/V-logo.svg"
										alt=""
									/>
								</div>
							</div>
							<div class="w-full xs:w-1/2 sm:w-1/3 px-4 mb-4">
								<div
									class="flex h-28 p-2 items-center justify-center bg-white rounded-3xl"
								>
									<img
										class="block mx-auto"
										src="/V-logo.svg"
										alt=""
									/>
								</div>
							</div>
							<div class="w-full xs:w-1/2 sm:w-1/3 px-4 mb-4">
								<div
									class="flex h-28 p-2 items-center justify-center bg-white rounded-3xl"
								>
									<img
										class="block mx-auto"
										src="/V-logo.svg"
										alt=""
									/>
								</div>
							</div>
							<div class="w-full xs:w-1/2 sm:w-1/3 px-4 mb-4">
								<div
									class="flex h-28 p-2 items-center justify-center bg-white rounded-3xl"
								>
									<img
										class="block mx-auto"
										src="/V-logo.svg"
										alt=""
									/>
								</div>
							</div>
							<div class="w-full xs:w-1/2 sm:w-1/3 px-4 mb-4">
								<div
									class="flex h-28 p-2 items-center justify-center bg-white rounded-3xl"
								>
									<img
										class="block mx-auto"
										src="/V-logo.svg"
										alt=""
									/>
								</div>
							</div>
							<div class="w-full xs:w-1/2 sm:w-1/3 px-4 mb-4">
								<div
									class="flex h-28 p-2 items-center justify-center bg-white rounded-3xl"
								>
									<img
										class="block mx-auto"
										src="/V-logo.svg"
										alt=""
									/>
								</div>
							</div>
							<div class="w-full xs:w-1/2 sm:w-1/3 px-4 mb-4">
								<div
									class="flex h-28 p-2 items-center justify-center bg-white rounded-3xl"
								>
									<img
										class="block mx-auto"
										src="/V-logo.svg"
										alt=""
									/>
								</div>
							</div>
							<div class="w-full xs:w-1/2 sm:w-1/3 px-4 mb-4">
								<div
									class="flex h-28 p-2 items-center justify-center bg-white rounded-3xl"
								>
									<img
										class="block mx-auto"
										src="/V-logo.svg"
										alt=""
									/>
								</div>
							</div>
							<div class="w-full xs:w-1/2 sm:w-1/3 px-4 mb-4">
								<div
									class="flex h-28 p-2 items-center justify-center bg-white rounded-3xl"
								>
									<img
										class="block mx-auto"
										src="/V-logo.svg"
										alt=""
									/>
								</div>
							</div>
							<div class="w-full xs:w-1/2 sm:w-1/3 px-4 mb-4">
								<div
									class="flex h-28 p-2 items-center justify-center bg-white rounded-3xl"
								>
									<img
										class="block mx-auto"
										src="/V-logo.svg"
										alt=""
									/>
								</div>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>
</section>
